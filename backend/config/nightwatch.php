...
return [
    // A package without an environment token must be inert. The vendor
    // default is enabled, which otherwise makes every request try the local
    // ingest socket before the Cloud agent has been connected.
    'enabled' => (bool) env('NIGHTWATCH_ENABLED', false)
        && filled(env('NIGHTWATCH_TOKEN')),
    ...
    'redact_headers' => explode(',', env(
        ...
        'Authorization,Cookie,Proxy-Authorization,X-XSRF-TOKEN,X-Support-Access'
    )),
