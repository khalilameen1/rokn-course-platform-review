...
<div class="admin-page animated fadeIn">
    @include('admin.partials.page-header', [
        ...
        'pageDescription' => 'أدخل فاتورة المزود وفترتها. الفاتورة المشتركة تخص المنصة ولا تُوزّع على الطلاب.',
    ...
    <div class="row mb-4">
        <div class="col-md-4"><div class="card admin-card h-100"><div class="card-body"><div class="text-muted">مجموع الفواتير النهائية المتاح بالجنيه</div><div class="h3 mb-0">{{ number_format($totals['actual_egp'], 2) }} ج.م</div></div></div></div>
        <div class="col-md-4"><div class="card admin-card h-100"><div class="card-body"><div class="text-muted">مبالغ غير نهائية — مستبعدة من التقرير</div><div class="h3 mb-0">{{ number_format($totals['estimated_egp'], 2) }} ج.م</div></div></div></div>
    ...
    <div class="card admin-card mb-4"><div class="card-header"><strong>فلترة ومراجعة الفواتير</strong></div><div class="card-body">
        ...
        @if($serviceSummary->isNotEmpty())
            <div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>الخدمة</th><th>نهائي</th><th>غير نهائي — مستبعد</th></tr></thead><tbody>@foreach($serviceSummary as $service => $summary)<tr><td>{{ \App\Models\OperatingCostPool::SERVICES[$service] ?? $service }}</td><td>{{ number_format($summary['actual_egp'], 2) }} ج.م</td><td>{{ number_format($summary['estimated_egp'], 2) }} ج.م</td></tr>@endforeach</tbody></table></div>
        @endif
    </div></div>
    ...
    <div class="card admin-card mb-4"><div class="card-header"><strong>{{ $editPool ? 'تعديل فاتورة التشغيل' : 'إضافة فاتورة تشغيل' }}</strong></div><div class="card-body">
        <form method="POST" action="{{ $editPool ? route('admin.operating-costs.update', $editPool) : route('admin.operating-costs.store') }}">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-4"><label>نطاق الفاتورة (الكورس فقط إذا كانت الفاتورة تخصه مباشرة)</label>
                    @if($editPool?->course_id && !$courses->contains('id', $editPool->course_id))
                        <input type="hidden" name="course_id" value="{{ $editPool->course_id }}"><div class="form-control-plaintext">{{ $editPool->course?->name_ar ?? 'كورس سابق' }} — نطاق تاريخي محفوظ</div>
                    @else
                        <select name="course_id" class="form-control"><option value="">مشتركة — كل المنصة</option>@foreach($courses as $course)<option value="{{ $course->id }}" @selected((string) old('course_id', $editPool?->course_id) === (string) $course->id)>{{ $course->name_ar }}{{ $course->trashed() ? ' (مؤرشف)' : '' }}</option>@endforeach</select>
                    @endif
                </div>
            </div>
            <div class="form-row">
                ...
                <input type="hidden" name="allocation_driver" value="{{ $editPool?->allocation_driver ?? 'active_students' }}">
            </div>
            <input type="hidden" name="is_final" value="0"><label><input type="checkbox" name="is_final" value="1" {{ old('is_final', $editPool?->is_final ?? true) ? 'checked' : '' }}> فاتورة نهائية مؤكدة من المزود</label>
            ...
            <button class="btn btn-primary" type="submit">{{ $editPool ? 'حفظ التعديل' : 'حفظ الفاتورة' }}</button>
    ...
    <div class="card admin-card"><div class="table-responsive"><table class="table table-hover mb-0">
        <thead><tr><th>الفاتورة</th><th>الفترة</th><th>النطاق</th><th>المبلغ</th><th>الحالة</th><th></th></tr></thead>
        <tbody>@forelse($pools as $pool)<tr>
            ...
            <td>{{ $pool->is_final ? 'نهائية' : 'غير نهائية — مستبعدة' }}</td>
        ...
        </tr>@empty<tr><td colspan="6" class="text-center text-muted py-4">لا توجد فواتير تشغيل بعد.</td></tr>@endforelse</tbody>
    </table></div>@if($pools->hasPages())<div class="card-footer">{{ $pools->links() }}</div>@endif</div>

    <div class="card admin-card mt-4"><div class="card-header"><strong>تقارير التشغيل والدخل حسب الكورس</strong></div><div class="card-body"><p class="text-muted">يعرض التقرير الدخل واستهلاك AI المؤكد وفواتير الكورس المباشرة. تكلفة الخدمات غير المنسوبة للطلاب أو الفئات تبقى غير متاحة.</p><div class="table-responsive"><table class="table table-hover mb-0"><thead><tr><th>الكورس</th><th>الطلاب النشطون</th><th></th></tr></thead><tbody>@forelse($courses as $course)<tr><td>{{ $course->name_ar }}</td><td>{{ number_format($course->active_enrollments_count) }}</td><td><a class="btn btn-sm btn-outline-primary" href="{{ $course->trashed() ? route('admin.courses.index', ['state' => 'archived', 'search' => $course->name_ar]) : route('admin.courses.show', [$course, 'tab' => 'commercial-report']).'#commercial-report' }}">فتح تقرير التشغيل</a></td></tr>@empty<tr><td colspan="3" class="text-center text-muted">لا توجد كورسات.</td></tr>@endforelse</tbody></table></div></div></div>
</div>
@endsection
