import {isAllowedRemoteUrl, usableAttachment} from './attachmentAccess';
import {
  hasLocalSpace,
  downloadPrivateFile,
  completedPrivateDownloadFolder,
  nativeDownloadId,
  cancelNativeDownloadIfActive,
  enqueueNativeDownload,
  cancelAttachmentTransfers,
} from './attachmentTransfers';
  ...
  if (Platform.OS === 'android' && NativeModules.RoknDownloads?.enqueue) {
    try {
      ...
      const nativeResult = await enqueueNativeDownload(
        {
          url: transferAttachment.url,
          title: currentAttachment.title,
          fileName,
          mimeType: mimeTypeFor(transferAttachment, fileName),
          stableKey: nativeStableKey(currentAttachment, operation.boundary.scope),
          expectedBytes: transferAttachment.fileSizeBytes || 0,
          external: Boolean(currentAttachment.external),
        },
